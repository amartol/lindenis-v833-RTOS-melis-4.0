...
static inline void twi_get_byte(const uint32_t base_addr, unsigned char  *buffer)
{
...
static inline void twi_get_last_byte(const uint32_t base_addr, unsigned char  *buffer)
{
...
static inline void twi_put_byte(const uint32_t base_addr, const unsigned char *buffer)
{
...
static inline void twi_disable(const uint32_t base_addr, unsigned int reg, unsigned int mask)
{
...
static inline void twi_enable(const uint32_t base_addr, unsigned int reg, unsigned int mask)
{
    unsigned int reg_val = readl(base_addr + reg);
...
static uint32_t twi_drv_query_irq_status(const uint32_t base_addr)
{
...
static void twi_drv_clear_irq_flag(uint32_t pending_bit, const uint32_t base_addr)
{
    uint32_t reg_val = readl(base_addr + TWI_DRIVER_INTC);
...
static void twi_clear_pending(const uint32_t base_addr)
{
    uint32_t reg_val = readl(base_addr + TWI_DRIVER_INTC);
...
/* start TWI transfer */
static void twi_start_xfer(const uint32_t base_addr)
{
 ...
 */
static void twi_set_rx_trig_level(uint32_t val, const uint32_t base_addr)
{
...
static void twi_set_packet_addr_byte(uint32_t val, const uint32_t base_addr)
{
...
static void twi_set_packet_data_byte(uint32_t val, const uint32_t base_addr)
{
...
static void twi_set_packet_cnt(uint32_t val, const uint32_t base_addr)
{
...
static void twi_enable_read_tran_mode(const uint32_t base_addr)
{
...
/* send slave_id + W */
static void twi_disable_read_tran_mode(const uint32_t base_addr)
{
    uint32_t mask = READ_TRAN;
    uint32_t reg_val = readl(base_addr + TWI_DRIVER_CTRL);
...
static inline void twi_soft_reset(const uint32_t base_addr, unsigned int reg, unsigned int mask)
{
    unsigned int reg_val = readl(base_addr + reg);
...
static void twi_enable_tran_irq(uint32_t bitmap, const uint32_t base_addr)
{
    uint32_t reg_val = readl(base_addr + TWI_DRIVER_INTC);
...
static void twi_disable_tran_irq(uint32_t bitmap, const uint32_t base_addr)
{
    uint32_t reg_val = readl(base_addr + TWI_DRIVER_INTC);
...
#if 0
static void twi_enable_dma_irq(uint32_t bitmap, const uint32_t base_addr)
{
    uint32_t reg_val = readl(base_addr + TWI_DRIVER_DMAC);
...
static void twi_disable_dma_irq(uint32_t bitmap, const uint32_t base_addr)
{
    uint32_t reg_val = readl(base_addr + TWI_DRIVER_DMAC);
...
static void twi_slave_addr(const uint32_t base_addr, twi_msg_t *msgs)
{
...
static int32_t twi_query_txfifo(const uint32_t base_addr)
{
    uint32_t reg_val;
...
static int32_t twi_query_rxfifo(const uint32_t base_addr)
{
    uint32_t reg_val;
...
static void twi_clear_txfifo(const uint32_t base_addr)
{
    uint32_t reg_val;
...
static void twi_clear_rxfifo(const uint32_t base_addr)
{
    uint32_t reg_val;
...
static int twi_send_msgs(hal_twi_t *twi, twi_msg_t *msgs)
{
    ...
    for (i = 0; i < msgs->len; i++)
    {
        ...
        if (time)
        {
            writeb(msgs->buf[i], twi->base_addr + TWI_DRIVER_SENDF);
        }
        else
        {
...
static uint32_t twi_recv_msgs(hal_twi_t *twi, twi_msg_t *msgs)
{
    ...
    for (i = 0; i < msgs->len; i++)
    {
        ...
        if (time)
        {
            msgs->buf[i] = readb(twi->base_addr + TWI_DRIVER_RECVF);
        }
        else
        {
            return 0;
...
#if 0
static int32_t twi_dma_write(twi_port_t twi_port, twi_msg_t *msgs)
{
    int32_t ret = 0;
    const uint32_t base_addr = g_twi_regbase[twi->port];
    ...
    twi->dma_using->dma_len = msgs->len;

    ret = twi_dma_xfer(twi, msgs->buf);
...
#if 0
static int32_t twi_dma_read(twi_port_t twi_port, twi_msg_t *msgs, int32_t num)
{
    int32_t ret = 0;
    ...
    twi->dma_using->dma_len = rmsgs->len;

    ret = twi_dma_xfer(twi, rmsgs->buf);
...
{
    int hal_complete_sem_ret;

    hal_complete_sem_ret = hal_sem_timedwait(twi->hal_complete_sem, twi->timeout * 100);
    if (hal_complete_sem_ret < 0)
    {
...
static int hal_twi_engine_complete(hal_twi_t *twi, int code)
{
    ...
    hal_complete_sem_ret = hal_sem_post(twi->hal_complete_sem);
    if (hal_complete_sem_ret != 0)
    {
        ret = SUNXI_TWI_FAIL;
...
static int hal_twi_drv_core_process(hal_twi_t *twi)
{
    ...
    int hal_complete_sem_ret, taskwoken = 0;
...
static int hal_twi_drv_do_xfer(hal_twi_t *twi, struct twi_msg *msgs, int num)
{
    ...
    if (num == 1)
    {
        if (msgs->flags & TWI_M_RD)
        {
            ...
            twi_set_packet_addr_byte(0, twi->base_addr);
#ifdef DMA_XXX
            if (twi->dma_rx && (msgs->len >= DMA_THRESHOLD))
            {
...
#ifdef DMA_XXX
            if (twi->dma_tx && (msgs->len >= DMA_THRESHOLD))
            {
                //              dprintk(DEBUG_INFO,
                //                      "master dma write",
                //                      twi->port);
...
#ifdef DMA_XXX
        if (twi->dma_rx && ((msgs + 1)->len >= DMA_THRESHOLD))
        {
...
twi_status_t hal_twi_xfer(twi_port_t port, twi_msg_t *msgs, int32_t num)
{
    ...
    hal_xfer_sem_ret = hal_sem_timedwait(twi->hal_xfer_sem, twi->timeout * 100);
    if (hal_xfer_sem_ret != 0)
    {
        TWI_ERR("[twi%d] is xfering, wait some time", twi->port);
...
static twi_status_t hal_twi_pinctrl_init(hal_twi_t *twi)
{
    ...
    ret = script_parser_fetch(phandle, twi_name, "twi_sck",
		    (int*)&gpio_cfg, sizeof(script_gpio_set_t) >> 2);
    if (ret != SCRIPT_PARSER_OK)
    {
	    TWI_ERR("parse twi_sck failed\n");
    }
    else
    {
	    twi->pin[0] = (gpio_cfg.port - 1) * 32 + gpio_cfg.port_num;
	    twi->pinmux = gpio_cfg.mul_sel;
    }

    ret = script_parser_fetch(phandle, twi_name, "twi_sda",
		    (int*)&gpio_cfg, sizeof(script_gpio_set_t) >> 2);
    if (ret != SCRIPT_PARSER_OK)
    {
	    TWI_ERR("parse twi_sda failed\n");
    }
    else
    {
	    twi->pin[1] = (gpio_cfg.port - 1) * 32 + gpio_cfg.port_num;
    }
    ...
    for (i = 0; i < TWI_PIN_NUM; i++)
    {
        ...
        {
            TWI_ERR("[twi] PIN set function failed!", twi->port);
            return TWI_STATUS_ERROR;
        }

        if (hal_gpio_set_driving_level(twi->pin[i], TWI_DRIVE_STATE))
        {
            TWI_ERR("[twi] PIN set driving failed!", twi->port);
            return TWI_STATUS_ERROR;
        }
...
static twi_status_t hal_twi_clk_init(hal_twi_t *twi)
{
    ...
    */
    ...
    //twi_set_clock(TWI->port, 24000000, TWI->fre/
...
twi_status_t hal_twi_init(twi_port_t port)
{
    ...
    phandle = script_get_handle();
    sprintf(twi_name, "twi%d", port);
    ret = script_parser_fetch(phandle, twi_name, "twi_frequency",
		    (int*)&twi->freq, 1);
    if (ret != SCRIPT_PARSER_OK)
    {
	 TWI_ERR("twi get frequency failed, use default frequency\n");
	 if (port == TWI_MASTER_0)
	 {
		twi->freq = TWI_FREQUENCY_400K;
	 }
	 else
	 {
		 twi->freq = TWI_FREQUENCY_200K;
	 }
    }
    ...
    if (hal_sem_create(&twi->hal_complete_sem, 0))
    {
        TWI_ERR("[twi%d] creating hal semaphore failed.", port);
	return TWI_STATUS_ERROR;
    }

    if (hal_sem_create(&twi->hal_xfer_sem, 1))
    {
        TWI_ERR("[twi%d] creating hal semaphore failed.", port);
        goto errtxsem;
    }
...
errregu:
    hal_sem_delete(twi->hal_xfer_sem);
errtxsem:
    hal_sem_delete(twi->hal_complete_sem);
...
const sunxi_hal_driver_twi_t sunxi_hal_twi_driver =
{
